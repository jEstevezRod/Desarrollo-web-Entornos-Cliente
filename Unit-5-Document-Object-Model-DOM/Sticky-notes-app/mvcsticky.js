...
class NotaView {
	constructor(note, tablero) {
		...
		this.title.setAttribute("maxlength", "15");
...
class Controlador {
	...
	crearNota() {
		...
		this.contID++;
		localStorage.setItem("ColeccionStorageNotas", JSON.stringify(this.coleccionNotas.totalNotas));
		
	}
	
	startCronometro() {
		this.control = setInterval(() => this.cronometro(), 60000);
	}
...
window.onload = () => {
	if (localStorage != "") {
		let retriew = localStorage.getItem("ColeccionStorageNotas");
		let datos = JSON.parse(retriew);
		controlador = new Controlador();
		if (datos != null) {
			for ( t; t < datos.length; t++ ) {
				controlador.coleccionNotas.totalNotas.push(datos[t]);
				var notaview = new NotaView(datos[t], ".tablero");
			}
		}
	} else {
		controlador = new Controlador();
